...
def noising(net, img, class_labels, num_steps=100, sigma_min=0.002, sigma_max=80, rho=7, w_corr=True, wo_last=False, device='cuda'):
...
@click.option('--resample_steps', 'num_resample_steps',      help='Number of sampling steps', metavar='INT',        type=click.IntRange(min=1), default=40, show_default=True)
...
def main(network_pkl, outdir, subdirs, subdirs_class, wo_last, seeds, class_idx, max_batch_size, num_resample_steps, path_to_label_distr=None, device=torch.device('cuda'), **sampler_kwargs):
    """Generate random images using the techniques described in the paper
    "Elucidating the Design Space of Diffusion-Based Generative Models".
