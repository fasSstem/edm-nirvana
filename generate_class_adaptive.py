...
def sampler(net, latents, class_labels, t_steps, sigma_first = torch.tensor(80.0, device='cuda'), sigma_last = torch.tensor(0.0, device='cuda')):
...
@click.option('--sigmas', 'sigmas_path',     help='diff class sigmas path', metavar='INT',                             type=str, required=True)
...
def main(network_pkl, outdir, subdirs, subdirs_class, seeds, class_idx, max_batch_size, sigmas_path, path_to_label_distr=None, device=torch.device('cuda'), **sampler_kwargs):
    """Generate random images using the techniques described in the paper
    "Elucidating the Design Space of Diffusion-Based Generative Models".
    ...
    for batch_seeds, cl, sigmas in tqdm.tqdm(zip(rank_batches, rank_classes, rank_sigmas), unit='batch', disable=(dist.get_rank() != 0)):
        torch.distributed.barrier()
